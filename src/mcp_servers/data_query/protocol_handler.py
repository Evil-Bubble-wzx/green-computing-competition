"""
MCP Protocol Handler — v2.0 SDK API

MCP SDK v2.0 使用 on_list_tools / on_call_tool 回调参数。
"""
...
class ProtocolHandler:
    ...
    def register_tool(self, name: str, description: str, input_schema: dict[str, Any], handler) -> None:
    ...
    async def execute_tool(self, name: str, arguments: dict[str, Any]) -> types.CallToolResult:
        ...
        try:
            ...
            if isinstance(result, str):
                return types.CallToolResult(
                    content=[types.TextContent(type="text", text=result)], isError=False
                )
            return types.CallToolResult(
...
def create_data_query_server(name: str, version: str, db_manager=None, query_engine=None):
    """创建 Data Query MCP Server (MCP SDK v2.0)"""
    handler = ProtocolHandler(name, version)

    # 注册所有工具
    from src.mcp_servers.data_query.tools import register_all
    register_all(handler, db_manager, query_engine)

    # v2.0 API: 通过构造函数参数注册回调
