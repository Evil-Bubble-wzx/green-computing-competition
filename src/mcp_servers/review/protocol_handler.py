...
class ProtocolHandler:
    ...
    async def execute_tool(self, name, args):
        ...
        if not t:
            return types.CallToolResult(content=[types.TextContent(type="text", text=f"未知: {name}")], isError=True)
        try:
            r = await t.handler(**args)
...
def create_review_server(name, version, pipeline=None, chat_engine=None, reviewer=None, settings=None):
    handler = ProtocolHandler(name, version)
    from src.mcp_servers.review.tools import register_all
    register_all(handler, pipeline, chat_engine, reviewer, settings)
